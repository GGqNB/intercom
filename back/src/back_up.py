...
MAIL_USERNAME = "andreyka.drogan@gmail.com"
MAIL_PASSWORD = "kjtu xeln ttvo lzgg"
MAIL_PORT = 465
...
@app.post("/send_email")
async def send_email(data: BaseDataPushSchemas):
    try:
        msg = MIMEMultipart()
...
async def make_call(apartment_number: int, caller_ws: WebSocket, hash_room: str):
...
async def call(apartment_number: int, hash_room: str):
    with connections_lock:
...
async def answer_call(apartment_number: int):
    with connections_lock:
        if apartment_number in call_tasks:
...
async def abort_call(apartment_number: int):
