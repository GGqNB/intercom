    ...
    try:
        while True:
            ...
            if data.lower() == "ping":  
                ...
                update_intercom_data(tech_name=get_user_id(websocket))
...
async def call(call_data: BaseCallData, api_key: APIKey = Depends(get_api_key),  session: AsyncSession = Depends(get_async_session),):
     ...
     with connections_lock:
         if call_data.apartment_number in call_tasks:
           ...
           print(flat_id)
           asyncio.create_task(make_call(flat_id, call_data.apartment_number,  intercom_ws, call_data.hash_room, call_data.blockDevice))
...
async def make_call(apartment_number: int, caller_ws: WebSocket, hash_room: str, blockDevice:BlockDevice, api_key: APIKey = Depends(get_api_key)):
    ...
    print(call_tasks[apartment_number])
    ...
    try:
        ...
        with connections_lock:
            for user_id, user_connection in connections.items():
                print(user_connection.apartment_number)
                if user_connection.role == "resident" and user_connection.apartment_number == apartment_number:
                    await safe_send_json(user_connection.websocket, 
                            {"type": "incoming_call", "from": "intercom", "apartment": apartment_number, "hash_room": hash_room, "block_device": blockDevice.model_dump()})

        try:
            await asyncio.wait_for(call_ended_event.wait(), timeout=10) 
...
@router_intercom_connect.get("/stown/open/{lock_id}/{code}/{local_url_base}") 
async def get_open_local_lock( 
        lock_id: str, code: str, local_url_base: str,
        api_key: APIKey = Depends(get_api_key),
        

): 
     try:
        proxy_resp = open_local_lock(lock_id=lock_id, code=code, local_url_base=local_url_base)
        return {
                "lock_id": lock_id,
                "code": code,
                "local_url_base": local_url_base,
